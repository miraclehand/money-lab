...
from yp_fin_utils.models.candle import CandleKR
...
class CandleFetcherKR(CandleFetcher):
    ...
    def fetch_candle_data(self, ticker: str = None):
        pass
    ...
    def fetch_and_upsert_candle_data(self, ticker: str = None, days: int = None):
        query = {'ticker': ticker} if ticker else {}
        stocks = StockKR.objects.raw(query).order_by([('capital', -1)])
        ...
        for start in range(0, total_stocks, self.BATCH_SIZE):
            ...
            for ohlcv_data in ohlcv_data_fetched:
                if ohlcv_data:
                    enhanced_ohlcv_data = self.calculate_price_changes(ticker, CandleKR, ohlcv_data)
    ...
    def _upsert_candle_data(self, ohlcv_data: list,  stock_instance_map: dict):
        ...
        try:
            candle = CandleKR.objects.get({'stock':stock_instance._id})
            stock_instance.new_adj_close = candle.update_ohlcv_with_adjustments(ohlcv_data)
            candle.save()
        except DoesNotExist:
            candle = CandleKR(
                stock=stock_instance,
                ohlcvs=[Ohlcv(ohlcv) for ohlcv in ohlcv_data]
            )
            candle.save()
